...
export const obterCotacao = async (ativo) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            console.log('Result:', result); // Log para verificar o resultado final
            resolve(result);
        } catch (error) {
            console.error('Erro ao obter cotação:', error);
            reject('Não foi possível obter a cotação no momento.');
        }
    });
}

export const traduzirFrases = async (frases) => {
    return new Promise(async (resolve, reject) => {
        try {
